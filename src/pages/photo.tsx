...
type DownloadUrlResponse = { downloadUrl: string; fileName: string };

export default function Photo() {
  const router = useRouter();
  const { download } = useDownloader();

  const { srcUrl, alt } = router.query as Record<string, string>;

  const {
    data,
    isValidating,
    error: swrError,
  } = useSWR<DownloadUrlResponse, AxiosError<ErrorResponse, unknown>>(
    `/api/download?srcUrl=${srcUrl}`,
    fetcher
  );
  ...
  if (swrError) {
    return (
      <>
        <div>{swrError.response?.data.message}</div>{' '}
        <button onClick={handleClick}>Click to download image</button>
      </>
    );
  }
  ...
  return (
    <main className={styles.main}>
      <button className={styles.button53} onClick={handleClick}>
